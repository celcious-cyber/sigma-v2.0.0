...
import { 
  CircleDollarSign, Activity, TrendingUp, TrendingDown,
  Wallet, 
...
const fetchStats = async () => {
  ...
  try {
    const res = await axios.get('/admin/flow/stats')
...
const kpis = computed(() => [
  { label: 'Total Kas Tersedia', key: 'balance_formatted', icon: Wallet, color: 'text-indigo-600', bg: 'bg-indigo-500/10', border: 'border-indigo-200', growth: stats.value.income_growth },
  { label: 'Total Pemasukan', key: 'income_formatted', icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-500/10', border: 'border-emerald-200', growth: stats.value.income_growth },
  { label: 'Total Pengeluaran', key: 'expense_formatted', icon: TrendingDown, color: 'text-rose-600', bg: 'bg-rose-500/10', border: 'border-rose-200', growth: '0%' },
  { label: 'Tagihan Tertunda', key: 'pending_formatted', icon: Activity, color: 'text-amber-600', bg: 'bg-amber-500/10', border: 'border-amber-200', growth: '0%' }
])

const distributionPercent = computed(() => {
  const total = stats.value.total_invoices || 1
  return {
    paid: Math.round((stats.value.status_distribution?.paid / total) * 100),
    unpaid: Math.round((stats.value.status_distribution?.unpaid / total) * 100),
    partial: Math.round((stats.value.status_distribution?.partial / total) * 100)
  }
})
</script>
  ...
  <div class="min-h-screen bg-[#F8FAFC] font-sans pb-20">
    <main class="max-w-[1600px] mx-auto p-8 lg:p-10 flex flex-col gap-8 animate-reveal">
        ...
        <div v-for="kpi in kpis" :key="kpi.label" 
          ...
          <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex flex-col items-end">
               ...
               <span class="text-[9px] font-bold text-emerald-500">{{ kpi.growth }}</span>
            </div>
          </div>
      ...
      <section class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <!-- Recent Transactions -->
        <div class="bg-white/60 backdrop-blur-2xl rounded-[3rem] p-10 border border-white/60 shadow-2xl">
        ...
        <div class="flex flex-col gap-8">
           <!-- Invoice Distribution Card -->
           <div class="bg-white/60 backdrop-blur-2xl rounded-[3rem] p-10 border border-white/60 shadow-xl">
              <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8">Status Pembayaran Faktur</h4>
              <div class="space-y-6">
                 <!-- Paid -->
                 <div class="space-y-2">
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest">
                       <span class="text-slate-600">Lunas</span>
                       <span class="text-emerald-600">{{ distributionPercent.paid }}%</span>
                    </div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                       <div class="h-full bg-emerald-500 rounded-full transition-all duration-1000" :style="`width: ${distributionPercent.paid}%`"></div>
                    </div>
                 </div>
                 <!-- Partial -->
                 <div class="space-y-2">
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest">
                       <span class="text-slate-600">Cicilan / Parsial</span>
                       <span class="text-amber-600">{{ distributionPercent.partial }}%</span>
                    </div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                       <div class="h-full bg-amber-500 rounded-full transition-all duration-1000" :style="`width: ${distributionPercent.partial}%`"></div>
                    </div>
                 </div>
                 <!-- Unpaid -->
                 <div class="space-y-2">
                    <div class="flex justify-between text-[10px] font-black uppercase tracking-widest">
                       <span class="text-slate-600">Belum Bayar</span>
                       <span class="text-rose-600">{{ distributionPercent.unpaid }}%</span>
                    </div>
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                       <div class="h-full bg-rose-500 rounded-full transition-all duration-1000" :style="`width: ${distributionPercent.unpaid}%`"></div>
                    </div>
                 </div>
              </div>
              <button 
                @click="router.push('/sigmaflow/reports')"
                class="w-full mt-10 py-4 rounded-[1.5rem] bg-slate-900 text-[10px] font-black text-white uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/10"
              >
           ...
           <div class="bg-white/60 backdrop-blur-2xl rounded-[3rem] p-10 border border-white/60 shadow-xl">
              <div class="flex items-center gap-3 mb-8">
                 ...
                 <h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400">Tunggakan Terbesar</h4>
              </div>
              
              <div class="space-y-5">
                 <div v-for="(a, i) in stats.top_arrears" :key="i" class="flex items-center justify-between group">
                    <div class="flex items-center gap-3">
                       <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-400">
                          {{ Number(i) + 1 }}
                       </div>
                       <div>
                          <p class="text-[11px] font-black text-slate-800 group-hover:text-indigo-600 transition-colors">{{ a.student_name }}</p>
                          <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ a.category }}</p>
                       </div>
                    </div>
                    <span class="text-[11px] font-black text-rose-600 italic tracking-tighter">{{ a.formatted }}</span>
                 </div>
                 
                 <div v-if="!stats.top_arrears?.length" class="py-6 text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                    Tidak ada tunggakan
                 </div>
              </div>
           </div>
